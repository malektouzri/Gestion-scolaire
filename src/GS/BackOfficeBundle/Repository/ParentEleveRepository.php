 ...
 */
class ParentEleveRepository extends \Doctrine\ORM\EntityRepository
{
    public function rechercheCin($id,$cin){
        $query = ParentEleveRepository::createQueryBuilder('p')
            ->where('p.id <> :id')
            ->andWhere('p.cin = :cin')
    ...
    public function nomP($nom){
        $query = ParentEleveRepository::createQueryBuilder('e')
    ...
    public function nomPren($nom,$id){
        $query = $this->getEntityManager()->createQuery('select e from BackOfficeBundle:ParentEleve e where
 e.idEcole = (select ec from BackOfficeBundle:Ecole ec where ec.id = :id) AND e.nom = :nom 
    ...
    public function nomPren2($nom,$prenom,$id){
                         ...
                         $query = $this->getEntityManager()->createQuery('select e from BackOfficeBundle:ParentEleve e
 where e.idEcole = (select ec from BackOfficeBundle:Ecole ec where ec.id = :id)
 AND e.nom = :nom or e.prenom = :nom OR e.prenom = :prenom OR e.nom = :prenom')
    ...
    public function alphaNum($annuler,$id){
              $query = $this->getEntityManager()->createQuery('select e from BackOfficeBundle:ParentEleve e
 where e.idEcole = (select ec from BackOfficeBundle:Ecole ec where ec.id = :id)
 AND e.annuler = :annuler ORDER BY e.nom')
